...
Image generation uses the OpenAI Image API, so the CLI needs outbound network access. In Claude Code, Bash commands require approval by default, and the permission system may prompt for confirmation before networked commands run.
